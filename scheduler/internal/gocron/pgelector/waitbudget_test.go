...
// a real-time-goroutine dependent assertion (the elector's heartbeat) bounded
// by a short wall-clock budget is a bet on machine load rather than an
// assertion about the code — so the shared const is applied uniformly.
//
...
// under t.Parallel), so budget × site count is an UPPER BOUND, not an exact
// figure — parallelism only lowers the real wall-clock sum. scheduler/internal/gocron
