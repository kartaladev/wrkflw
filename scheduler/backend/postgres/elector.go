// Package postgres provides the Postgres-backed leader [scheduler.Elector] for
...
// scheduler façade neutral of any DB driver.
package postgres
...
// Elector is the Postgres-backed leader [scheduler.Elector]. Beyond IsLeader it
// exposes Close (so scheduler.Scheduler.Close releases its dedicated connection)
...
// Postgres-backed leader [scheduler.Elector]: exactly one replica wins the leader
...
//
// Pass the returned value to scheduler.WithElector. Its dedicated connection is
// released by scheduler.Scheduler.Close (which closes the elector when it
