...
// forbiddenDBDeps are the database-driver packages the public scheduler façade
// must never pull in — directly OR transitively. DB coupling lives in
// scheduler/backend/{postgres,mysql} and the persistence-lock bridge; the façade
...
// TestSchedulerHasNoDBDeps asserts that the public scheduler package carries no
...
// deep. This is what lets a consumer (e.g. the service façade) import scheduler
// without polluting its dependency graph with a DB driver.
func TestSchedulerHasNoDBDeps(t *testing.T) {
	cfg := &packages.Config{Mode: packages.NeedImports | packages.NeedDeps}
	pkgs, err := packages.Load(cfg, "github.com/kartaladev/wrkflw/scheduler")
	...
	for _, root := range pkgs {
		...
		for imp := range root.Imports {
			if isForbidden(imp) {
				t.Fatalf("scheduler must not directly import a DB driver, found: %s", imp)
			}
		}
		...
		if path, found := findForbiddenInClosure(root); found {
			t.Fatalf("scheduler must not transitively import a DB driver, reached via: %s",
				strings.Join(path, " -> "))
		}
	}
}
