...
```
engine (pure)              runtime (adapter)                 ports / impls
─────────────              ─────────────────                 ─────────────
1. emits a Command   ───►  2. Runner.perform() fulfils it ─► a PORT (Scheduler,
   (names a value type,       via an injected port             TaskStore, Publisher,
    never does I/O)                                            CallLinkStore, …)
                                                                     │
                           4. Runner.Deliver(trigger) ◄──── 3. the port produces a
   engine.Step advances        re-enters the engine             Trigger (later, async,
   the parked token            (journalled, CAS-guarded)        or in the same call)
```
...
```
engine                        runtime                          humantask (model+ports)
──────                        ───────                          ───────────────────────
KindUserTask node
   │ emits
   ▼
engine.AwaitHuman ──────────► Runner.perform()
  {TaskToken, Eligibility}      ├─ resolver.Candidates(...)  ── humantask.ActorResolver
                                └─ tasks.Upsert(HumanTask{}) ── humantask.TaskStore
actor claims task
   └────────────────────────► TaskService.Claim(token, actor)
                                ├─ store.Get(token)          ◄─ humantask.TaskStore
                                ├─ authz.Authorize(task.Eligibility, actor, task.Vars)
                                └─ engine.NewHumanClaimed(...) ─┐ Trigger
                                Runner.Deliver(trigger) ◄───────┘ → Step advances token
```
...
timer node / deadline
   │ emits
   ▼
engine.ScheduleTimer ───────► Runner.perform() → armTimer(...)
  {TimerID, FireAt, Kind}       └─ sched.Schedule(id, fireAt, fire) ─► (MemScheduler |
                                                                        gocron-backed)
   ⏰ fireAt reached            fire() callback ◄──────────────────────┘
                                 ├─ engine.NewTimerFired(now, id)
                                 └─ Runner.Deliver(...) ─► Step advances the token
```
...
```
(a) INBOUND — external caller / transport                runtime                    engine
────────────────────────────────────────                ───────                    ──────
DeliverMessage(name, corrKey, payload)
        │
        ▼
   findMessageWaiter(name, corrKey) ──► r.msgWaiters  (synced after each deliverLoop;
        │                                              no Store enumeration API)
        ├─ not found → clean no-op
        └─ found(instanceID)
             ├─ engine.NewMessageReceived(now, name, corrKey, payload)
             └─ Runner.Deliver(def, instanceID, trg) ──► Step fires the
...
```
(b) OUTBOUND — engine                    runtime                        SignalBus
────────────────────                    ───────                        ─────────
throw/signal node
   │ emits
   ▼
engine.ThrowSignal ────────────────────► Runner.perform()
  {Name, Payload}                          └─ sigbus.Publish(name, payload) ─► fan-out to
                                                                                Subscribe(id, name)
                          deliver(ctx, id, trg) ◄──────────────────────────────┘ subscribers
                            └─ Runner.Deliver(...) back into each waiting instance

engine.SendMessage ────────────────────► Runner.perform() → return nil, nil
  {Name, CorrelationKey, Payload}          (emitted as a message.<Name> outbox row inside
                                            the SAME commit tx — at-least-once, see flow 7)
```
...
```
(b) CALL ACTIVITY — SYNC             runtime (Runner.perform)              engine (parent)
────────────────────────             ────────────────────────              ───────────────
KindCallActivity node
   │ emits (parent token parks on cmdID)
   ▼
StartSubInstance ──────────────────► depth guard (cycle protection)
  {cmdID, DefRef, Input(copy)}         │  reg.Lookup(DefRef) → childDef
                                       │  childSt := r.Run(childCtx, childDef, childID, Input)
                                       │     (child shares store/journal/outbox/catalog/
                                       │      scheduler; drives its OWN deliverLoop)
                                       └─ switch childSt.Status:
                                            Completed → NewSubInstanceCompleted(cmdID, vars) ─┐
                                            Running   → NewSubInstanceFailed (parked child)   ├─► resume
                                            Failed    → NewSubInstanceFailed (status N)       ─┘  parent token
```
...
```
(c) CALL ACTIVITY — ASYNC   runtime (Runner.perform)      CallLinkStore     CallNotifier        engine (parent)
─────────────────────────   ────────────────────────      ─────────────     ────────────       ───────────────
StartSubInstance ─────────► depth = LookupChild(parent)?+1
  {cmdID, DefRef, Input}     │  guard maxCallDepth
                             │  link := CallLink{child, parent, ParentCommandID, depth}
                             │  runChild(childDef, childID, Input, &link)
                             │     link threaded into child's first Create tx ─► CallLink row (persisted)
                             └─ return nil, nil   (parent STAYS parked; no sync trigger)

  ...child runs independently (own deliverLoop, own timers/human tasks)...
  child reaches terminal → AppliedStep.CallOutcome → link flipped to PENDING-notify

CallNotifier.DrainOnce (background):
   ClaimPending(batch) ───────────────────────────────────► claims links with an outcome
   for each: reg.Lookup(ParentDef)
             trg = Completed ? NewSubInstanceCompleted(ParentCommandID, Output)
                             : NewSubInstanceFailed(ParentCommandID, Err)
             deliver(parentDef, ParentInstanceID, trg) ─────────────────────────────────────► resume parent
             MarkNotified(child)   (ErrTokenNotFound = parent already resumed = success)
```
...
```
Runner.deliverLoop (one applied trigger)
────────────────────────────────────────
Step produces (newState, commands)
   │
   ├─ terminalOutboxEvent(prevStatus, st, cmds)   (runtime/outbox.go)
   │     StatusCompleted  → OutboxEvent{Topic:"instance.completed",  Payload: vars}
   │     StatusFailed     → OutboxEvent{Topic:"instance.failed",     Payload:{error}}
   │     StatusTerminated → OutboxEvent{Topic:"instance.terminated", Payload:{error}}
   │
   ├─ outboundMessageEvents(st, cmds)             (each engine.SendMessage command)
   │     → OutboxEvent{Topic:"message."+Name, Payload:{messageName, correlationKey, variables}}
   │
   └─ AppliedStep{ State, Trigger, Events:[…] , TimerArms, TimerCancels, … }
          │
          ▼
     Store.Commit(expected Token, step)   ── ATOMIC: snapshot + journal + outbox rows
          (Postgres/MySQL: writeOutbox INSERTs each event into wrkflw_outbox
           in the same tx; optional NOTIFY wrkflw_outbox on commit — ADR-0022)
```
...
A background `Relay` (`internal/persistence/postgres/relay.go`, mirror in
`mysql/`) polls the outbox and pushes each row to the consumer-supplied
`runtime.Publisher`. The consumer builds the `Publisher` by wrapping any watermill
`message.Publisher` with `eventing.NewPublisher`.

```
Relay.Run (poll ticker + optional LISTEN/NOTIFY wake)     Publisher            broker
──────────────────────────────────────────────────       ─────────            ──────
loop: DrainOnce(ctx)                                (eventing.NewPublisher
  │                                                  over a watermill pub)
  ├─ BEGIN tx
  ├─ SELECT … FROM wrkflw_outbox
  │     WHERE status='pending' AND next_attempt_at<=now
  │     ORDER BY id FOR UPDATE SKIP LOCKED LIMIT batch   ◄─ cooperative claim
  │                                                          (multi-relay safe)
  ├─ for each claimed row:
  │     pub.Publish(ctx, event) ──────────────────────────────────► watermill → broker
  │        success → UPDATE status='published', published_at=now
  │        failure → retry_count++, next_attempt_at=now+backoff, last_error=…
  │                  retry_count >= MaxDeliveryAttempts → status='dead' (quarantine)
  └─ COMMIT tx  (whole batch commits atomically; a row is 'published'
                 only AFTER a successful Publish → at-least-once)
```
...
- **LISTEN/NOTIFY (optional, `WithListenNotify`).** The write side can `NOTIFY
  wrkflw_outbox` on commit; the relay holds a dedicated connection that `LISTEN`s
  and drains immediately, coalescing bursts (`drainUntilEmpty`). The poll ticker
  stays as a fallback for missed notifications, restarts, and multi-worker fan-out.
