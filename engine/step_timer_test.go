...
func deadlineDef() *model.ProcessDefinition {
	return &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			event.NewStart("start"),
			activity.NewUserTask("userTask", activity.WithCandidateRoles("manager"), activity.WithWaitDeadline(schedule.AfterExpr(`"3h"`), "escalate"), activity.WithDeadlineAction("notify")),
...
func reminderDef() *model.ProcessDefinition {
	return &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			event.NewStart("start"),
			activity.NewUserTask("userTask", activity.WithCandidateRoles("manager"), activity.WithWaitDeadline(schedule.AfterExpr(`"3h"`), "escalate"), activity.WithDeadlineAction("notify"), activity.WithWaitAction(schedule.EveryExpr(`"1h"`), "remind")),
			event.NewEnd("normalEnd"),
			event.NewEnd("escalateNode"),
		},
		Flows: []flow.SequenceFlow{
...
func TestInWaitReminderNoActionEmitsNothingOnFire(t *testing.T) {
	...
	def := &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			event.NewStart("start"),
			activity.NewUserTask("userTask", activity.WithCandidateRoles("manager"), activity.WithWaitAction(schedule.EveryExpr(`"1h"`), "")),
			event.NewEnd("end"),
		},
		Flows: []flow.SequenceFlow{
			{ID: "f1", Source: "start", Target: "userTask"},
...
func TestActionFailedCancelsOutstandingTimers(t *testing.T) {
	...
	def := &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			...
			activity.NewUserTask("userTask", activity.WithCandidateRoles("manager"), activity.WithWaitDeadline(schedule.AfterExpr(`"3h"`), "esc"), activity.WithDeadlineAction("notify")),
