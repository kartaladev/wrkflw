...
// step_harvest_terminal_admission_test.go — on the terminal snapshot the
...
//
//   - engine/step_terminal_dispatch_test.go (TestTerminalDispatchOutcomes) is the
...
//   - THIS table is black-box and runs on the harvested substrate: a terminal snapshot whose
...
func TestOnlyAPlainFullRollbackIsAdmittedOnAHarvestedTerminalInstance(t *testing.T) {
	...
	require.Nil(t, terminal.Scopes, "control: the harvest closed every scope")
	...
	cases := []testCase{
		...
		{
			// ⚠ Reclassified from allowOnTerminal to rejectSilently. Unguarded
