...
// step_harvest_incident_retirement_test.go — the harvest retires the
...
//
// The mechanism, and why the shape is this specific. The engine deliberately
...
// The harvest makes handleUnhandledError's records-exist predicate true for a record
...
//
// ⚠ That last clause corrects an earlier account, which attributed the retirement to
...
// The end numbers hold; the named mechanism does not, and the assertion below pins the
// measured one.
//
// Measured on THIS fixture, by deleting both harvest call sites and re-running the drive
// (the pre-harvest control) against the shipped behaviour:
//
...
//
// ⚠ An earlier account records tokens 2 → 0 for the same property. That count is its
// own fixture's; here the fail-fast branch leaves three tokens because three branches
// are live. The load-bearing pair — incidents 1 → 0 with no compensation dispatched →
// undoX dispatched — reproduces exactly. Counts do not travel between fixtures; the property does.
//
...
//     branch whose behaviour the harvest changes.
//
...
func TestHarvestRetiresTheSurvivingTokensIncident(t *testing.T) {
	...
	// ⚠ MEASURED, and it corrects an earlier account, which named removeOrphanedIncidents
	// as what retires the incident, which would put the retirement at the walk's FINISH
	// (that function is called only from endInstance). It is already gone here, one Step
	...
	// The end numbers that account records are right; the named mechanism is not.
	...
	assert.Empty(t, final.Incidents,
		"ACCEPTED CONSEQUENCE, pinned deliberately: the incident that was "+
			"preserved on this shape is now retired, so runtime/outbox.go's "+
