...
// fall-through instead of through this table.
type timerRecord struct {
	...
	// TimerCompensationStall record. It is what makes a LATE fire
...
// if no such record exists. An empty timerID names no record.
...
// empty key names no record and cancels nothing; an empty
...
//
// An empty taskID cancels NOTHING. A task id is an identity; the empty
...
//
// An empty tokenID names no token. excludeTimerID is NOT guarded.
...
// ActionFailed) is deferred to the errors/compensation work.
...
// see [TimerKind.detectionOnly]. Such a record is a detection
...
// is forward work the harness must fire for the backoff to end.
//
// ⚠ It is deliberately NOT a len(s.Timers) test, and a consumer must not
// re-derive it as one. It once was, and four of the five timer-arm
