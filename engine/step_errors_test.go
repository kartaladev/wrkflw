...
func actionFailedBoundaryDef() *model.ProcessDefinition {
	nestedDef := &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			event.NewStart("inner-start"),
			activity.NewServiceTask("inner-svc", activity.WithTaskAction("work-action")),
...
func directBoundaryOnInnerSvcInSubprocessDef() *model.ProcessDefinition {
	nestedDef := &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			...
			activity.NewServiceTask("inner-recover", activity.WithTaskAction("inner-recover-action")),
			event.NewEnd("inner-end"),
