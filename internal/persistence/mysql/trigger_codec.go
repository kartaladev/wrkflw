// Package mysql is the internal MySQL-backed persistence implementation.
package mysql
...
// triggerEnvelope is the flat JSON shape written to wrkflw_journal.trigger_.
...
func MarshalTrigger(t engine.Trigger) ([]byte, string, error) {
	...
	default:
		return nil, "", fmt.Errorf("workflow-persistence-mysql: marshal trigger: unhandled variant %T", t)
	}
	data, err := json.Marshal(env)
	if err != nil {
		return nil, "", fmt.Errorf("workflow-persistence-mysql: marshal trigger: %w", err)
	}
...
func UnmarshalTrigger(kind string, data []byte) (engine.Trigger, error) {
	...
	if err := json.Unmarshal(data, &env); err != nil {
		return nil, fmt.Errorf("workflow-persistence-mysql: unmarshal trigger %q: %w", kind, err)
	}
	switch kind {
	...
	default:
		return nil, fmt.Errorf("workflow-persistence-mysql: unmarshal trigger: unknown kind %q", kind)
	}
}
