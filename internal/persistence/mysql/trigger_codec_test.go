...
// MarshalTrigger for each table case and cross-checks it against
// mypkg.AllTriggerKinds.
func TestTriggerCodecRoundTrip(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"ActionFailed": {
			...
			assert: func(t *testing.T, got engine.Trigger) {
				...
				require.Equal(t, 0.0, got.(engine.ActionFailed).JitterFraction)
			},
		},
		"ResolveIncident": {
	...
	for name, tc := range tests {
		data, kind, err := mypkg.MarshalTrigger(tc.in)
		...
		got, err := mypkg.UnmarshalTrigger(kind, data)
	...
	require.ElementsMatch(t, mypkg.AllTriggerKinds, gotKinds,
		"test table kinds do not match mypkg.AllTriggerKinds: declared=%v, got=%v",
		mypkg.AllTriggerKinds, gotKinds)
}
...
func TestActionFailedJitterRoundTrip(t *testing.T) {
	...
	data, kind, err := mypkg.MarshalTrigger(in)
...
func TestActionFailedJitterBackwardCompat(t *testing.T) {
	...
	got, err := mypkg.UnmarshalTrigger("action_failed", oldPayload)
	require.NoError(t, err)
	af, ok := got.(engine.ActionFailed)
	require.True(t, ok)
	...
	require.True(t, af.Retryable)
	require.Equal(t, 0.0, af.JitterFraction)
}
