...
| `persistence/chaining_e2e_test.go`, `happy_path_vars_carry` (`inst-a`) | yes (genuine) | not an instance; **basis corrected #134** | **The basis recorded here until #134 was WRONG, though the verdict was right — the third time this document has caught that exact pattern in itself.** It said the site was safe because `Chainer.Handle` records the link BEFORE `Drive`. That dependency does not exist at HEAD: applying this document's own method — move the `Record` block below `Drive` in `runtime/chain/chainer.go`, insert 200 ms in the gap — leaves all three dialects **12/12 green** (measured twice, independently, during #134's review). The real reason is the #86 refactor: `awaitChainedSuccessor` (`:200-211`) polls **both** asserted facts in ONE conjunction and captures `st, link` inside the closure, so every later assertion reads values the wait already established. Production write order is irrelevant to it. **Why the stale basis was dangerous rather than merely untidy:** it failed in the UNSAFE direction. Un-refactor the wait back to a single `store.Load` and the old basis would still be true, so the row would still read "not an instance" while the site had genuinely become one. If the conjunction at `:200-211` is ever split, re-open this row. (#134 also changed `wireChainerRunner` from a `go cr.Run(...)` goroutine to `Chainer.Start`; that removed a *delivery* race — an envelope dropped before any subscription existed — which is a different class from this one and does not bear on the verdict.) |
| `persistence/chaining_e2e_test.go`, `branch_routing` (`inst-b`) | yes (genuine) | not an instance; **basis corrected #134** | As above — and note that "as above" now points at a REPLACED basis. This row is safe for the #86 conjunction reason, not for the production-write-order reason it used to inherit. **Re-open this row if the conjunction at `:200-211` is ever split** — measured during #134's review: splitting it fails every wait-using scenario on every dialect, this one included. |
| `persistence/chaining_e2e_test.go`, `idempotency` (`inst-a-idem`) | no | not an instance; **basis corrected #134** | As above, with a correction of its own. This row used to add that "under the reorder-plus-delay it failed on both …" — **that no longer reproduces, and as written it contradicted row 1 outright**, since the same experiment leaves all three dialects green. Both statements described the PRE-#86 test; only row 1's was updated when #86 landed, so this table asserted opposite results for one experiment until #134. Safe for the #86 conjunction reason. **Re-open if the conjunction at `:200-211` is ever split** — measured: splitting it fails this scenario too. |
...
second signal, or because the wait polls both asserted facts in one conjunction
and hands them out of the closure. (This sentence used to end "because production
code orders the writes (`Record` before `Drive`)" — the same superseded basis the
`branch_routing` row carried, found by following what pointed AT that basis
rather than by searching for a symbol. It is corrected here for the same reason
and by the same measurement.) That count is already generous to the check, because it assumes the
