...
4. **Richer resource modeling for Privileges** — `AuthzSpec.Privileges` today is a `[]string` carrying `"resource:action"` tokens; the `casbinauthz` façade splits on `:` into `(obj, act)` for `Enforce`. Richer modeling (domains/tenants, object hierarchies, wildcard policies) is a follow-up once the DB adapter provides a policy-management API.
