...
> **State:** work lives on branch **`feat/builtin-actions`** (HEAD `74a9e9c`, base `main` `67e2c01`) — **NOT merged, NOT pushed** (maintainer chose "keep branch as-is" for review before integration). `go build ./...` clean, `go test -race ./...` 27 pkgs / 0 failures / 0 races (incl. mailpit testcontainers integration + in-process TLS round-trips), `golangci-lint run ./...` 0 issues. **engine/ + model/ are zero-diff vs main (verified).** **Next free ADR: 0076** (0074 + 0075 consumed). Ledger: `.superpowers/sdd/progress.md` (sections "Built-in service actions" + "Transform I/O + deferred-items refactor"). Spec: `docs/specs/2026-06-29-builtin-actions-design.md`. Plan: `docs/plans/2026-06-29-builtin-actions.md`. Two final opus whole-branch reviews (original 8-task program + the continuation) both "Ready to merge", 0 Critical/Important.
>
...
> 2. **`action/httpcall`** — `NewHTTPCall(opts...)`; `net/http`; `WithBaseURL/Method/Header/HTTPClient/BodyKey/OutputKeys/URLExpr`; retry (4xx exc 408/429 → NonRetryable; 408/429/5xx/transport → retryable); outputs `httpStatus/httpBody/httpHeaders`.
...
> Reference wiring: **`examples/builtin_actions/main.go`** runs all four end-to-end. ADRs **0074**/**0075** (0075 updated for the I/O extensions). Action coverage 90.4% (email 86.4, httpcall 90.1, others 100; all >85%).
>
> **🟢 NEXT SESSION — remaining (non-blocking, all Minor) follow-ups:**
