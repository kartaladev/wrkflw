...
| `service/` — transport-agnostic facade | `service.New(runner, lister, reg) *Service` — eight operations: `StartInstance`, `GetInstance`, `DeliverMessage`, `ClaimTask`, `ReassignTask`, `CompleteTask`, `CancelInstance`, `ListInstances`. Resolves `ProcessDefinition` by `DefID:DefVersion` from the registry; passes all instance ops through `runtime.Runner`. Fully usable without any transport import. | Task 2 |
| `transport/rest` — stdlib HTTP handler | `rest.NewHandler(svc, ...Option) http.Handler` — stdlib `*http.ServeMux` (no third-party router), mountable under `http.StripPrefix`. Routes: `POST /instances`, `GET /instances/{id}`, `POST /instances/{id}/message`, `POST /instances/{id}/tasks/{taskID}/claim`, `POST /instances/{id}/tasks/{taskID}/reassign`, `POST /instances/{id}/tasks/{taskID}/complete`, `POST /instances/{id}/cancel`, `GET /instances` (keyset-paginated list). `WithInstanceMapper(fn)` customizes the `InstanceResponse` shape applied to ALL instance-returning endpoints. `WriteHTTPError(w, err)` maps sentinel errors to HTTP status codes. | Task 3 |
...
3. **REST deny-body Content-Type** — `WriteHTTPError` writes plain-text bodies for 403 admin-deny;
   consumers expecting `application/json` errors need a custom error body option.
