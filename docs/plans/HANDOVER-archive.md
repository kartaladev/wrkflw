# wrkflw — Handover ARCHIVE (frozen 2026-07-30)

> **This file is history. Do not read it for current state and do not append to it.**
>
> Live handover state is `docs/plans/HANDOVER.md`.
>
> This document was the rolling handover from 2026-06-20 to **2026-07-08**, at which
> point it stopped being maintained: its newest resume point knows ADR-0114, while the
> repo reached ADR-0159 — 333 commits and 45 ADRs later. It is kept because the
> per-sub-project sections below are an accurate record of how those deliveries were
> reasoned about, and several explain decisions still load-bearing today.
>
> It died of its own format: append-only, twenty-odd stacked "PREVIOUS RESUME POINT"
> blocks, and an opening instruction to "read top to bottom" that became impossible.
> Its replacement is deliberately short and is rewritten in place, never appended to.
...
> unqualified `NewCatalog`. Constructor-only rename (~111 sites); `MapCatalog` type + concrete return kept.
> Unique-token global replace, no collision. Spec/plan under `docs/{specs,plans}/2026-07-08-rename-newmapcatalog*`.
>
...
>
> **What shipped (ADR-0087 — 9-task plan + two rounds of /code-review follow-ups, all merged):**
> 1. **8-package split.** `runtime/kernel` = value types + port interfaces + in-memory reference impls (`MemStore`/`CachingStore`/`MemScheduler`/`MemCallLinkStore`/`MemTimerStore`/`MapDefinitionRegistry`/`CachingDefinitionRegistry` + all sentinels) — the leaf everything imports. Behavioural pkgs: `runtime/signal` (`SignalBus`), `runtime/calllink` (`CallNotifier`), `runtime/chain` (`Chainer`), `runtime/task` (`TaskService`), `runtime/monitor` (`LineageReader` + outbox/timer stats + dead-letter). `runtime/view` = `InstanceSnapshot`/`ActionableView` DTOs + `StatusString` (independent leaf, imports only engine/model/humantask — **no kernel**). Root `runtime` is now a thin driver. Import direction: `kernel ← {calllink,chain,signal,task,monitor}`; `kernel,signal ← runtime`; `view` independent.
> 2. **Driver rename (BREAKING, pre-v0.1.0).** `Runner`→`ProcessDriver`, `NewRunner`→`NewProcessDriver`; the 1,175-line `runner.go` split into per-concern `processdriver_*.go` files (+ `armTimer`/`RehydrateTimers` into `timerops.go`). OTel span names + instrumentation scope kept stable via new `kernel.InstrumentationScope` const (de-dups the string across driver/task/calllink).
> 3. **Clock-option rename (BREAKING).** The 5 single-per-package clock options collapse to `WithClock` (`runtime`/`signal`/`calllink`/`chain`/`task`); kernel's 4 (`WithCachingStoreClock`/`WithMemCallLinkClock`/`WithCachingDefinitionRegistryClock`/`WithMemSchedulerClock`) **kept distinct** because pkg kernel has multiple option families (a single `WithClock` would collide).
> 4. **Test-support consolidation.** New `runtime/internal/runtimetest` (exported `Must*` ctors + `RecordingScheduler`/`FixedJitter` doubles + `SignalCatchDef`/`TimerIntermediateDef`/`ApprovalDef` fixtures + `SeedCallLink`/`SeedTerminalCallLink`) replaces ~280 lines of per-package duplication. The former **public** `MemCallLinkStore.Seed`/`SeedTerminal` test-only methods were **REMOVED** — seeding now drives the public `MemStore.Create/Commit` path from runtimetest, keeping test-only surface off the shipped API.
>
> **Reviews:** two whole-diff opus `/code-review`s (decomposition merge + WithClock merge) — **0 correctness bugs** (finders confirmed byte-identical behaviour modulo qualifiers/renames). All findings were quality/doc/altitude and are **resolved**: ticker-loop test rigor rewrite, `kernel.InstrumentationScope` de-dup, and stale godoc/test-name fixes (persistence facades `runtime.WithClock`→`calllink.WithClock`; `TestWithChainClockNilFallsBackToSystem`→`TestWithClockNilFallsBackToSystem`).
>
> **Not done (deliberate, logged):** package-name stutter rename `signal.SignalBus`/`task.TaskService` (the plan intentionally kept type names unchanged; would require further breaking type renames).

## ⏩ PREVIOUS RESUME POINT — updated 2026-07-03 (P1-C persistence / migration ops)

> **State:** ✅ **MERGED + PUSHED to `origin/main`** (merge `07b4ec1` + follow-up `9bc91d1`; branch deleted). _(Superseded by the runtime-decomposition block above; kept for history.)_ 6 SDD tasks; final whole-branch opus review **READY** (0 Critical/Important). `go build ./...` clean; `go test -race ./...` all packages pass / 0 races; `golangci-lint run ./...` **0 issues**; `engine/`+`model/` **zero-diff**; coverage persistence 88.0% / store 87%. **Next free ADR: 0087** (0085 + 0086 consumed). Ledger: `.superpowers/sdd/progress.md`. Spec: `docs/specs/2026-07-03-persistence-migration-ops.md`. Plan: `docs/plans/2026-07-03-persistence-migration-ops.md`.
>
> **What shipped (P1-C from the 2026-06-30 production-readiness backlog — full track in one branch):**
> 1. **Migration lifecycle API (ADR-0085).** Internal stateless `store.Migrator` over goose `Provider` (per-call provider construction; pg uses a `stdlib.OpenDBFromPool` shim whose Close never closes the pool). Public `persistence.Migrator` interface + `MigrationStatus` DTO (goose types confined to internal) + `NewPostgresMigrator`/`NewMySQLMigrator`/`NewSQLiteMigrator` → `(Migrator, error)` (ADR-0083 fail-fast, typed-nil guarded). Methods: `Up/UpByOne/UpTo/Down/DownTo/Version/Status/HasPending`. **Rollback (`Down`/`DownTo`) exposed plainly with loud DATA-LOSS godoc** (operator owns DB). Back-compat `Migrate`/`MigrateMySQL`/`MigrateSQLite` now thin `NewXMigrator(...).Up` wrappers. Re-exported `persistence.ErrNilDependency`. Reference CLI `examples/migrate` (stdlib flag, `run()` seam).
> 2. **Schema-parity guardrail.** `internal/persistence/store/migration_parity_test.go` (`package store_test`) migrates PG(9)/MySQL(2)/SQLite(1) to head and asserts a LOGICAL schema (table+column+nullable+PK) convergence — physical types excluded. Two legit normalizations: SQLite PK-rowid NOT-NULL quirk + MySQL `trigger_` reserved-word alias (sourced from `dialect.JournalTriggerColumn()`). Proven to bite on perturbation.
> 3. **Production-safety (ADR-0086).** `docs/production-checklist.md` (pool sizing, statement-timeout/isolation, 3 opt-in MUST-DOs) + opt-in `persistence.WarnUnsafeConfig(logger, DeploymentProfile)` (consumer-invoked; NO auto constructor-time warn — ADR-0086 records why). Cross-linked from README + retention.md.
> 4. **`PruneTimers` promoted onto the public `persistence.Pruner` interface** (store already implemented it).
>
> **Deferred Minors (from final review, non-blocking):** M1 `NewPruner(pool)` single-return doc shorthand in checklist+retention.md (pre-existing style); M2 redundant `tc := tc` in `persistence/migrator_test.go` (cosmetic, Go 1.25 loopvar); Task-1 Minors #3–#5 (Migrator `with`-helper inconsistency / unreachable `default` branch / curried-func micro-alloc — all intentional/defensive).

## ⏩ PREVIOUS RESUME POINT — updated 2026-07-03 (constructor conventions + builder/loader unification)
...
>
> **✅ INTEGRATION DONE:** merged to `main` (`ae79bc4`, `--no-ff`) and pushed to `origin/main` on 2026-06-30. The previously-held backlog-follow-ups commits were pushed in the same operation, so `origin/main` is now fully in sync (0 ahead). The only remaining items are the 4 cosmetic Minors above. `feat/builtin-actions` can be deleted at your convenience.
>
> ---
