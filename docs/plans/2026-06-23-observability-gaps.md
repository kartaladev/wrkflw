...
**Context:** `postgres.Store{pool, historyCap, notify}`; `NewStore(pool, opts ...StoreOption)`. `Load(ctx,id)` and `Commit(ctx,expected,step)` are uninstrumented. Mirror `relay.go`: it holds `logOpt/tpOpt/mpOpt observability.Option` + `tel observability.Telemetry`, builds `tel = observability.New("github.com/zakyalvan/krtlwrkflw/persistence", filterNilOpts(...))` in `NewRelay`, and uses `r.tel.Tracer.Start(...)`. `observability.New(...).Float64Histogram(name, desc)` creates a histogram. Read relay.go's `WithRelayTracerProvider`/`filterNilOpts` + the relay_observability_test.go SDK-recorder setup.
