...
`persistence.NewAdvisoryLockOwnership` and pass the resulting `InstanceOwnership` to
`kernel.NewCachingInstanceStore` so that only one replica acts on each completed child.
...
cachingStore, _ := kernel.NewCachingInstanceStore(store, ownership)
