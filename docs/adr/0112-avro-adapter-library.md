...
a third-party validation library directly. ADR-0111 landed the sibling `validation/jsonschema`
adapter following that pattern.
...
3. Serialize back to the same `.avsc` text so `DescribableStrategy.Descriptor()` round-trips
   through the wire/YAML definition form, matching the `validation/jsonschema` and
   `validation/expr` adapters' convention.

## Decision

We adopt **`github.com/linkedin/goavro/v2`**, confined entirely to `validation/avro` (this
package) — the `definition`/engine core imports neither goavro nor any other Avro library.
...
avro schema: %w`, consistent with `validation/jsonschema`'s `Validate` wrapping its schema
compiler's error the same way.
...
- One new third-party dependency (`github.com/linkedin/goavro/v2`, pulling in
  ...
  but it is isolated behind `validation/avro`; swapping it for a different Avro library touches
  only this package, not `definition`/engine core or the other adapters (`validation/expr`,
  `validation/jsonschema`, `validation/callback`).
- The conformance check is **encode-based**: it validates by attempting to serialize the input
  ...
  `validation/jsonschema` gets from its compiler).
