...
9. **Adversarial audit of the design bundle (mandatory, ONE checkpoint per delivery).** Once **all** design documents for a delivery are written — the spec (`docs/specs/`), its ADR(s) (`docs/adr/`), **and** the plan (`docs/plans/`) — run **one** adversarial audit over the whole bundle together, **before implementation starts**. Not per-document, not per-stage: one audit per delivery bundle. The same trigger applies when a **new ADR or plan is later written for an existing spec** — audit that updated bundle before acting on it. Dispatch one or more subagents — **use the Opus model for audit agents** — briefed to *attack* the documents, not summarize them. The brief: hunt for holes, unstated assumptions, internal contradictions, cross-document inconsistencies (plan vs spec vs ADR), claims that contradict the actual codebase (**source-verify every factual claim**), missing failure modes / edge cases / migration gaps — and propose a concrete fix for each finding. Adjudicate the findings (do **not** auto-apply them), fold the accepted fixes into the documents, then proceed to implementation. A bundle that has not survived its audit is not an input to implementation.
    ...
    ⚠⚠ **When a revision changes MORE THAN ONE decision, an INTERACTION PASS is a separate, mandatory piece of work — dedicate a lens to it.** Fixing N decisions and re-auditing is **not** the same as auditing N fixed decisions: each fix changes the premises the *other* decisions were written against, and nothing else in this rule forces anyone to re-derive that. The B3 authz/security bundle is the case. Its first audit found 58 issues in *individual* decisions; the revision fixed all of them and **failed a second audit on the interactions**, where **five of nine Criticals were holes the revision's own fixes opened in each other** — the hoisted spec gate emptied the escape hatch another decision pointed at; the new `reassign` privilege landed in a dimension a third decision made deny; the tri-state that removed upgrade-stranding on one field had it re-introduced on another by a fourth. Each fix was correct alone.

    The brief for this lens is not "attack the documents" but **"take the changed decisions pairwise and derive what each does to the other's premises"**. Give it the explicit list of what changed. It composes with the other lenses; it does not replace them.

    ⚠ **Corollary for the author, before the audit is even dispatched:** if a revision touches more than one decision, write down the pairwise consequences yourself and mark the ones you could not resolve. An unwritten interaction is the cheapest possible Critical.
...
13. **Expand every reference on first use — a bare label is not an explanation.** This repo is dense with short identifiers that carry no meaning on their own: `D4`, `F7`, `C-12`, `backlog 103`, `blocker 5`, `ADR-0175`, `phase 6`, `item 54`, `E2`. When reporting, summarizing, adjudicating, or asking a question, **the first mention of any such label in that message must carry a one-clause gloss of what it actually is** — *"D4 (the strict-reference rule that denies a predicate referencing an absent variable)"*, *"backlog 103 (deny-list ABAC predicates allow when the variable is missing)"*, *"blocker 5 (the pgx notifier test whose 'load-flaky' label is unverified)"*. Thereafter the bare label is fine within the same message.

    This binds hardest exactly where it is most tempting to skip: **findings tables, audit adjudications, handover documents and progress reports**, where dozens of labels appear together and the author has all of them in working memory. The reader does not. A table row reading *"F12 — MAJOR — confirmed"* is worthless to the next session; *"F12 — oversize bodies classify as 400 not 413, because every decode site already wraps in `ErrBadInput` — MAJOR, confirmed"* is actionable.

    The same applies to **numbers and file references**: say what a count counts (*"29 test pins that assert the body-derived actor"*, not *"29 pins"*), and what a cited symbol does, not only where it lives. If a gloss would be long, give the short version and point at the document that holds the full one — but never emit the bare label alone.
