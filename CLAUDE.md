...
10. **Handover checkpoint — mandatory the moment a bundle is implementation-ready.** As soon as a delivery's design bundle has survived its rule-#9 audit (and again whenever a phase completes or a session ends), **stop and hand the work off** before writing code: refresh the auto-memory (the `MEMORY.md` index line **and** a topic/handover file) and add a **`▶ Progress`** block at the top of the plan recording branch + commit SHA, which phases landed, what remains, source-verified still-true facts, and the exact verification commands. The test is: *could a fresh session with no transcript pick this up and implement it?* Implementation is expected to be delegated to such a session — a delivery whose state lives only in the current transcript is **not** handed over, and unhanded-over work must not be left at a session boundary.
