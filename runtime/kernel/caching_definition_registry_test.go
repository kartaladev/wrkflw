...
func TestCachingDefinitionRegistry_ImplementsInterface(t *testing.T) {
	var _ kernel.DefinitionRegistry = (*kernel.CachingDefinitionRegistry)(nil)
	t.Log("CachingDefinitionRegistry satisfies kernel.DefinitionRegistry")
}
...
func TestNewCachingDefinitionRegistryFailsFast(t *testing.T) {
	...
	type testCase struct {
		...
		ttl     time.Duration
		assert  func(t *testing.T, c *kernel.CachingDefinitionRegistry, err error)
	}
	cases := []testCase{
		{
			name:    "nil backing",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			c, err := kernel.NewCachingDefinitionRegistry(tc.backing, tc.ttl)
			tc.assert(t, c, err)
		})
	}
}
