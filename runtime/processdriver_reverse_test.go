...
func reverseFixtureDef() *model.ProcessDefinition {
	return &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			...
			activity.NewUserTask("approve1", activity.WithEligibleRoles("manager")),
			...
			activity.NewUserTask("approve2", activity.WithEligibleRoles("manager")),
			event.NewEnd("end"),
		},
...
func reverseTargetVarsFixtureDef() *model.ProcessDefinition {
	return &model.ProcessDefinition{
		...
		Nodes: []model.Node{
			...
			activity.NewUserTask("approve2", activity.WithEligibleRoles("manager")),
			activity.NewServiceTask("next", activity.WithTaskAction("next"), activity.WithCompensateAction("unnext")),
			activity.NewUserTask("approve3", activity.WithEligibleRoles("manager")),
			event.NewEnd("end"),
		},
		Flows: []flow.SequenceFlow{
			{ID: "f1", Source: "start", Target: "approve1"},
