...
func TestTerminalOutboxEvent(t *testing.T) {
	tests := map[string]struct {
		...
		// Kind is left at its zero value, engine.IncidentAction — what every
		// incident stored by an older version decodes to, and the one kind the
		// cause-of-death allow-list admits.
