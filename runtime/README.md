...
> thunk registered with `scheduler.WithJobStore` and requires no explicit call
> from your code. An explicit `RehydrateTimers` call is only needed when you
> inject a consumer-owned scheduler via `WithScheduler` and want to manage
> rehydration yourself. See [`examples/scenarios/timer_durability`](../examples/scenarios/timer_durability)
