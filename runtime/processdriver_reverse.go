...
// reversing while a compensation walk is already in flight — defense in depth
// against the TOCTOU window between this facade's pre-check Load and the
// engine's own Step, so this facade's guards are not the only line of defense.
//
