...
func TestWithMemCallLinkClockNilFallsBackToSystem(t *testing.T) {
	cl := runtime.NewMemCallLinkStore(
		runtime.WithMemCallLinkLease("replica-X", 30*time.Second),
		runtime.WithMemCallLinkClock(nil), // must be ignored; clock.System() must survive
	)
	...
	_, err = store.Commit(t.Context(), tok, runtime.AppliedStep{
		State:       done,
		Trigger:     startTrg(),
		CallOutcome: &runtime.CallOutcome{Completed: true},
	})
	require.NoError(t, err)
