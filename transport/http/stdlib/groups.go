...
func (c InstanceRoutes) Customize(mux *http.ServeMux, opts ...httpcore.CustomizeOption[*http.ServeMux]) {
	...
	handle(r, inst, cfg, http.MethodPost, "/instances", func(w http.ResponseWriter, req *http.Request) {
		...
		status, body, err := httpcore.StartInstance(req.Context(), c.Svc, in, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper))
		if err != nil {
	...
	handle(r, inst, cfg, http.MethodGet, "/instances/{id}", func(w http.ResponseWriter, req *http.Request) {
		id := req.PathValue("id")
		status, body, err := httpcore.GetInstance(req.Context(), c.Svc, id, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper))
		if err != nil {
			writeErr(cfg, w, req, err)
			return
		}
	...
	handle(r, inst, cfg, http.MethodGet, "/instances/{id}/snapshot", func(w http.ResponseWriter, req *http.Request) {
		id := req.PathValue("id")
		status, body, err := httpcore.GetInstanceSnapshot(req.Context(), c.Svc, id, httpcore.DisclosingProjection(req.Context(), cfg.RequestActor, cfg.Disclosure), httpcore.WithholdDefinition(req.Context(), cfg.RequestActor, cfg.Disclosure))
		if err != nil {
			writeErr(cfg, w, req, err)
			return
		}
	...
	handle(r, inst, cfg, http.MethodGet, "/instances/{id}/actionable", func(w http.ResponseWriter, req *http.Request) {
		id := req.PathValue("id")
		status, body, err := httpcore.GetActionableView(req.Context(), c.Svc, id, httpcore.DisclosingProjection(req.Context(), cfg.RequestActor, cfg.Disclosure), httpcore.WithholdDefinition(req.Context(), cfg.RequestActor, cfg.Disclosure))
		if err != nil {
			writeErr(cfg, w, req, err)
			return
		}
	...
	handle(r, inst, cfg, http.MethodPost, "/instances/{id}/signals", func(w http.ResponseWriter, req *http.Request) {
		...
		status, body, err := httpcore.DeliverSignal(req.Context(), c.Svc, id, in, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper))
		if err != nil {
			writeErr(cfg, w, req, err)
			return
		}
...
func (c TaskRoutes) Customize(mux *http.ServeMux, opts ...httpcore.CustomizeOption[*http.ServeMux]) {
	...
	handle(r, inst, cfg, http.MethodPost, "/tasks/{token}/claim", func(w http.ResponseWriter, req *http.Request) {
		...
		status, body, err := httpcore.ClaimTask(req.Context(), c.Svc, token, in, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper)), actor)
		if err != nil {
			writeErr(cfg, w, req, err)
			return
		}
	...
	handle(r, inst, cfg, http.MethodPost, "/tasks/{token}/complete", func(w http.ResponseWriter, req *http.Request) {
		...
		status, body, err := httpcore.CompleteTask(req.Context(), c.Svc, token, in, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper)), actor)
		if err != nil {
			writeErr(cfg, w, req, err)
			return
		}
	...
	handle(r, inst, cfg, http.MethodPost, "/tasks/{token}/reassign", func(w http.ResponseWriter, req *http.Request) {
		...
		status, body, err := httpcore.ReassignTask(req.Context(), c.Svc, token, in, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper)), actor)
		if err != nil {
			writeErr(cfg, w, req, err)
			return
		}
...
func (c AdminRoutes) Customize(mux *http.ServeMux, opts ...httpcore.CustomizeOption[*http.ServeMux]) {
	...
	handle(r, inst, cfg, http.MethodPost, "/admin/instances/{id}/incidents/{incidentID}/resolve",
		func(w http.ResponseWriter, req *http.Request) {
			...
			status, body, err := httpcore.ResolveIncident(req.Context(), c.Svc, instanceID, incidentID, in, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper))
			if err != nil {
				writeErr(cfg, w, req, err)
				return
			}
	...
	handle(r, inst, cfg, http.MethodPost, "/admin/instances/{id}/compensation/resolve-stall",
		func(w http.ResponseWriter, req *http.Request) {
			...
			status, body, err := httpcore.ResolveCompensationStall(req.Context(), c.Svc, instanceID, in, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper))
			if err != nil {
				writeErr(cfg, w, req, err)
				return
			}
	...
	handle(r, inst, cfg, http.MethodPost, "/admin/instances/{id}/cancel",
		func(w http.ResponseWriter, req *http.Request) {
			instanceID := req.PathValue("id")
			status, body, err := httpcore.CancelInstance(req.Context(), c.Svc, instanceID, httpcore.DisclosingMapper(req.Context(), cfg.RequestActor, cfg.Disclosure, cfg.InstanceMapper))
			if err != nil {
				writeErr(cfg, w, req, err)
				return
			}
