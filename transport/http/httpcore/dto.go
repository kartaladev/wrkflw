...
// the correlated instance or a message-start event, so no def_ref is
// carried.
...
// request context, never a value the caller supplies — an earlier version of this
// struct carried an Actor and any caller could name their own roles.
//
// The type is kept rather than dropped so a body posted by an older client still
...
// reassignment is resolved from the request context, not from the body.
...
// time, which cost an extra query.
