...
func TestNewScheduler_SatisfiesPortAndFires(t *testing.T) {
	...
	s, err := scheduling.NewScheduler(scheduling.WithClock(fakeClock))
	...
	cases := []tc{
		{
			...
			assert: func(t *testing.T) {
				...
				s, err := scheduling.NewScheduler(scheduling.WithClock(clk), scheduling.WithLogger(logger))
				require.NoError(t, err)
				t.Cleanup(func() { _ = s.Close() })
		...
		},
		{
			...
			assert: func(t *testing.T) {
				clk := clockwork.NewFakeClock()
				s, err := scheduling.NewScheduler(scheduling.WithClock(clk), scheduling.WithLogger(nil))
				require.NoError(t, err)
				t.Cleanup(func() { _ = s.Close() })
				require.NotNil(t, s)
			},
	...
	cases := []tc{
		{
			...
			assert: func(t *testing.T) {
				...
				s, err := scheduling.NewScheduler(scheduling.WithClock(clk), scheduling.WithTracerProvider(tp))
				require.NoError(t, err)
				t.Cleanup(func() { _ = s.Close() })
				require.NotNil(t, s)
			},
		},
		{
			...
			assert: func(t *testing.T) {
				...
				s, err := scheduling.NewScheduler(scheduling.WithClock(clk), scheduling.WithMeterProvider(mp))
				require.NoError(t, err)
				t.Cleanup(func() { _ = s.Close() })
				require.NotNil(t, s)
			},
